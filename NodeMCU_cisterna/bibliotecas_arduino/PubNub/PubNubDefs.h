...
retry:
     ...
     * timeout is. */
    // Se d_origin não estiver definido isso ir causar um erro na funcao connect.
    // Confira se foi feita uma chamada a PubNub.begin(publish_key, subscribe_key, origin)
    DBGprint("d_origin: ");
    DBGprintln(d_origin);
